# -*- coding: utf-8 -*-
"""
Coletor diário de notícias via Google News RSS, organizado por setor da B3.

Como funciona:
1. Para cada setor configurado (petróleo, mineração, agro, etc), monta URLs
   do RSS com parâmetros locais brasileiros (pt-BR, BR)
2. Para cada termo do setor, busca o feed e lê os resultados
3. Resolve a URL real de cada link (segue redirect do Google)
4. Traduz todos os títulos para inglês
5. Salva tudo num Word organizado por setor, depois por termo,
   no formato 'Title (Source, May 20)'

Cobertura setorial:
- Petróleo e Gás (Petrobras, PRIO, Brava, PetroRecôncavo)
- Metais e Mineração (Vale, CSN, Gerdau, Usiminas, Aura, Ferbasa)
- Distribuição de Combustíveis (Vibra, Raízen, Ultrapar, Cosan)
- Gás Natural (Comgás, Compass)
- Agricultura, Milho e Soja (SLC, BrasilAgro, 3tentos, Boa Safra, AgroGalaxy)
- Fertilizantes (Heringer, Vittia)
- Petroquímicos (Braskem, Unipar)
- Papel e Celulose (Suzano, Klabin, Irani, Eucatex)

Dependências: pip install feedparser python-docx requests deep-translator

"""

import feedparser  # type: ignore
import urllib.parse
import requests
import os
from datetime import datetime
from email.utils import parsedate_to_datetime
from concurrent.futures import ThreadPoolExecutor, as_completed
from docx import Document
from docx.oxml.ns import qn
from docx.oxml import OxmlElement
from deep_translator import GoogleTranslator


# ============================================================
# CONFIGURAÇÃO (edite aqui)
# ============================================================

# Parâmetros de busca comuns a todos os setores. Como o foco é o
# mercado brasileiro, mantemos sempre pt-BR. A tradução para inglês
# é feita depois, no titulo das notícias.
HL = "pt-BR"
GL = "BR"
CEID = "BR:pt-419"
TRADUZIR = True

SETORES = {
    "Petróleo e Gás": {
        "termos": [
            '"Petrobras" PETR4',
            '"PRIO" PRIO3',
            '"Brava Energia" BRAV3',
            '"PetroRecôncavo" RECV3',
            "preço petróleo Brent",
            "ANP leilão petróleo",
            "pré-sal produção",
        ],
    },
    "Metais e Mineração": {
        "termos": [
            '"Vale" VALE3 mineração',
            '"CSN Mineração" CMIN3',
            '"Gerdau" GGBR4',
            '"Usiminas" USIM5',
            '"CSN" CSNA3 siderurgia',
            '"Aura Minerals" AURA33',
            '"Ferbasa" FESA4',
            "preço minério de ferro",
            "preço aço China demanda",
        ],
    },
    "Distribuição de Combustíveis": {
        "termos": [
            '"Vibra Energia" VBBR3',
            '"Raízen" RAIZ4',
            '"Ultrapar" UGPA3 Ipiranga',
            '"Cosan" CSAN3',
            "ANP distribuição combustíveis",
            "preço gasolina diesel Brasil",
            "RenovaBio CBIO",
        ],
    },
    "Gás Natural": {
        "termos": [
            '"Comgás" CGAS5',
            '"Compass" Cosan gás natural',
            "nova lei do gás Brasil",
            "mercado livre gás natural",
            "tarifa gás canalizado",
        ],
    },
    "Agricultura (Milho e Soja)": {
        "termos": [
            '"SLC Agrícola" SLCE3',
            '"BrasilAgro" AGRO3',
            '"3tentos" TTEN3',
            '"Boa Safra" SOJA3',
            '"AgroGalaxy" AGXY3',
            "safra soja Brasil Conab",
            "safra milho Brasil",
            "preço soja Chicago CBOT",
            "exportação soja China",
        ],
    },
    "Fertilizantes": {
        "termos": [
            '"Heringer" FHER3 fertilizantes',
            '"Vittia" VITT3',
            "preço fertilizante ureia Brasil",
            "importação fertilizantes Rússia",
            "Plano Nacional Fertilizantes",
        ],
    },
    "Petroquímicos": {
        "termos": [
            '"Braskem" BRKM5',
            '"Unipar" UNIP6',
            "preço eteno polietileno",
            "antidumping petroquímico Brasil",
            "nafta resina",
        ],
    },
    "Papel e Celulose": {
        "termos": [
            '"Suzano" SUZB3 celulose',
            '"Klabin" KLBN11',
            '"Irani" RANI3 papel',
            '"Eucatex" EUCA4',
            "preço celulose BHKP exportação China",
            "embalagem papelão Brasil",
        ],
    },
}

JANELA_TEMPORAL = "when:1d"
PASTA_SAIDA = os.path.dirname(os.path.abspath(__file__))
RESOLVER_LINKS_REAIS = True
IDIOMA_DESTINO = "en"
MAX_THREADS = 10

USER_AGENT = (
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
    "(KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
)


# ============================================================
# COLETA
# ============================================================

def monta_url_rss(termo: str) -> str:
    query_completa = f"{termo} {JANELA_TEMPORAL}".strip()
    query_codificada = urllib.parse.quote(query_completa)
    return (
        f"https://news.google.com/rss/search?q={query_codificada}"
        f"&hl={HL}&gl={GL}&ceid={CEID}"
    )


def coleta_noticias_termo(termo: str) -> list[dict]:
    url = monta_url_rss(termo)
    feed = feedparser.parse(url, agent=USER_AGENT)

    if feed.bozo and not feed.entries:
        print(f"    Aviso: feed com problema para '{termo}'. Pulando.")
        return []

    noticias = []
    for entry in feed.entries:
        titulo = getattr(entry, "title", "") or ""
        link = getattr(entry, "link", "") or ""
        if not titulo:
            continue
        fonte = ""
        if hasattr(entry, "source") and hasattr(entry.source, "title"):
            fonte = entry.source.title
        noticias.append({
            "titulo_original": titulo,
            "titulo_en": titulo,
            "link": link,
            "fonte": fonte,
            "data_raw": getattr(entry, "published", ""),
            "data_formatada": "",
        })
    return noticias


def resolve_link_real(url_google: str) -> str:
    try:
        resposta = requests.head(
            url_google,
            allow_redirects=True,
            timeout=10,
            headers={"User-Agent": USER_AGENT},
        )
        # Alguns servidores recusam HEAD — tenta GET sem baixar o corpo
        if resposta.status_code == 405:
            resposta = requests.get(
                url_google,
                allow_redirects=True,
                timeout=10,
                headers={"User-Agent": USER_AGENT},
                stream=True,
            )
            resposta.close()
        return resposta.url
    except Exception:
        return url_google


def resolve_links_em_paralelo(noticias: list[dict]) -> list[dict]:
    if not noticias:
        return noticias
    with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
        futuros = {
            executor.submit(resolve_link_real, n["link"]): n
            for n in noticias
            if n["link"]
        }
        for futuro in as_completed(futuros):
            noticia = futuros[futuro]
            try:
                noticia["link"] = futuro.result()
            except Exception:
                pass
    return noticias


# ============================================================
# TRADUÇÃO E DATA
# ============================================================

def traduz_texto(texto: str) -> str:
    try:
        return GoogleTranslator(source="auto", target=IDIOMA_DESTINO).translate(texto)
    except Exception as erro:
        print(f"      Falha ao traduzir, mantendo original. Motivo: {erro}")
        return texto


def traduz_em_paralelo(noticias: list[dict]) -> list[dict]:
    if not noticias:
        return noticias
    with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
        futuros = {
            executor.submit(traduz_texto, n["titulo_original"]): n
            for n in noticias
        }
        for futuro in as_completed(futuros):
            noticia = futuros[futuro]
            try:
                noticia["titulo_en"] = futuro.result()
            except Exception:
                noticia["titulo_en"] = noticia["titulo_original"]
    return noticias


def formata_data_curta(data_raw: str) -> str:
    if not data_raw:
        return ""
    try:
        dt = parsedate_to_datetime(data_raw)
        return dt.strftime("%b %d")
    except Exception:
        return ""


# ============================================================
# GERAÇÃO DO WORD
# ============================================================

def adiciona_hiperlink(paragrafo, url: str, texto: str):
    part = paragrafo.part
    r_id = part.relate_to(
        url,
        "http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink",
        is_external=True,
    )

    hyperlink = OxmlElement("w:hyperlink")
    hyperlink.set(qn("r:id"), r_id)

    novo_run = OxmlElement("w:r")
    rPr = OxmlElement("w:rPr")

    cor = OxmlElement("w:color")
    cor.set(qn("w:val"), "0563C1")
    rPr.append(cor)

    sublinhado = OxmlElement("w:u")
    sublinhado.set(qn("w:val"), "single")
    rPr.append(sublinhado)

    novo_run.append(rPr)

    elemento_texto = OxmlElement("w:t")
    elemento_texto.text = texto
    novo_run.append(elemento_texto)

    hyperlink.append(novo_run)
    paragrafo._element.append(hyperlink)


def gera_arquivo_word(resultados: dict, caminho_saida: str):
    doc = Document()

    data_hoje = datetime.now().strftime("%d/%m/%Y")
    doc.add_heading(f"Notícias setoriais do dia, {data_hoje}", level=0)

    total = sum(
        len(noticias)
        for termos_dict in resultados.values()
        for noticias in termos_dict.values()
    )
    paragrafo_resumo = doc.add_paragraph()
    run_resumo = paragrafo_resumo.add_run(
        f"Total de {total} notícias coletadas em {len(resultados)} setores. "
        f"Títulos traduzidos para inglês."
    )
    run_resumo.italic = True

    for setor, termos_dict in resultados.items():
        doc.add_heading(setor, level=1)

        for termo, noticias in termos_dict.items():
            doc.add_heading(termo, level=2)

            if not noticias:
                paragrafo_vazio = doc.add_paragraph()
                run_vazio = paragrafo_vazio.add_run(
                    "Nenhuma notícia encontrada para este termo."
                )
                run_vazio.italic = True
                continue

            for noticia in noticias:
                paragrafo = doc.add_paragraph(style="List Bullet")
                if noticia["link"]:
                    adiciona_hiperlink(paragrafo, noticia["link"], noticia["titulo_en"])
                else:
                    paragrafo.add_run(noticia["titulo_en"])

                partes_sufixo = []
                if noticia["fonte"]:
                    partes_sufixo.append(noticia["fonte"])
                if noticia["data_formatada"]:
                    partes_sufixo.append(noticia["data_formatada"])

                if partes_sufixo:
                    sufixo = f" ({', '.join(partes_sufixo)})"
                    paragrafo.add_run(sufixo)

    doc.save(caminho_saida)


# ============================================================
# ORQUESTRAÇÃO
# ============================================================

def main():
    inicio = datetime.now()
    print(f"[{inicio.strftime('%H:%M:%S')}] Iniciando coleta setorial de notícias")

    resultados = {}

    for nome_setor, setor_cfg in SETORES.items():
        print(f"\n>> Setor: {nome_setor}")
        resultados[nome_setor] = {}

        for termo in setor_cfg["termos"]:
            print(f"  Buscando: {termo}")
            noticias = coleta_noticias_termo(termo)

            if RESOLVER_LINKS_REAIS and noticias:
                noticias = resolve_links_em_paralelo(noticias)

            if TRADUZIR and noticias:
                print(f"    Traduzindo {len(noticias)} títulos para inglês...")
                noticias = traduz_em_paralelo(noticias)

            for n in noticias:
                n["data_formatada"] = formata_data_curta(n["data_raw"])

            resultados[nome_setor][termo] = noticias
            print(f"    {len(noticias)} resultados prontos")

    nome_arquivo = f"noticias_setoriais_{datetime.now().strftime('%Y%m%d')}.docx"
    caminho = os.path.join(PASTA_SAIDA, nome_arquivo)
    gera_arquivo_word(resultados, caminho)

    duracao = (datetime.now() - inicio).total_seconds()
    print(f"\nArquivo salvo em: {caminho}")
    print(f"Tempo total: {duracao:.1f} segundos")


if __name__ == "__main__":
    main()
